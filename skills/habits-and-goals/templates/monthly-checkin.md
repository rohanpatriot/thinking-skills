# Monthly Check-in Template

Structured review for the last Friday of each month.

## Contents

- [Purpose](#purpose)
- [Instructions for Claude](#instructions-for-claude)
- [Check-in Template](#check-in-template)
- [Monthly Check-in Schedule](#monthly-check-in-schedule)
- [Reminders](#reminders)

---

## Purpose

The 1-in-60 Rule from aviation: A 1-degree error in navigation results in being 1 mile off course for every 60 miles flown.

Applied to annual planning:
- Small misalignments compound over time
- Monthly reviews catch drift early
- Course corrections keep you on track

**Timing:** Last Friday of each month

---

## Instructions for Claude

When you receive a monthly check-in request, guide the user through the three core questions using the `AskUserQuestion` tool.

### Session Flow

#### Opening

"Let's do your monthly check-in. This takes about 15-20 minutes and ensures you're still on track for the year."

#### Question 1: Big Goal Alignment

**Ask (via AskUserQuestion tool):**
- Question: "Looking at your life right now, what really matters most to you?"
- Options: Let me reflect on this | Same as when I set my goals | Things have shifted | Help me think through this

**Follow-up (via AskUserQuestion tool):**
- Question: "Are your Big Goals still aligned with what matters most, or has something shifted?"
- Options: Still aligned | Need minor adjustments | Major reassessment needed | Not sure

**If adjustments needed (via AskUserQuestion tool):**
- Question: "What specifically needs to change?"
- Options: Adjust a Big Goal | Add a new goal | Remove a goal | Change priority order

#### Question 2: Daily Systems Assessment

**Ask (via AskUserQuestion tool):**
- Question: "Think about your Daily Systems over the past month. Are they creating the momentum you need toward your Big Goals?"
- Options: Yes, strong momentum | Somewhat, but could be better | No, not working | Haven't been consistent

**Follow-up if inconsistent (via AskUserQuestion tool):**
- Question: "What got in the way? What would help you be more consistent next month?"
- Options: Time constraints | Energy/motivation | Systems too ambitious | Life circumstances | Need accountability

**Follow-up if not working (via AskUserQuestion tool):**
- Question: "Do the systems need to change, or is it an execution problem?"
- Options: Change the systems | Fix execution | Both | Not sure, help me diagnose

**Follow-up if working well (via AskUserQuestion tool):**
- Question: "Any refinements that would make them even more effective?"
- Options: Increase intensity (more A days) | Add a new system | Keep as is | Simplify further

#### Question 3: Anti-Goal Check

**Ask (via AskUserQuestion tool):**
- Question: "Look at your Anti-Goals—the things you refuse to sacrifice. Are you in danger of violating any of them?"
- Options: All good, staying within guardrails | Yellow flag, need to watch this | Red flag, already violating | Not sure, need to reflect

**Follow-up if issues (via AskUserQuestion tool):**
- Question: "What specific Anti-Goal is at risk? What would course correction look like?"
- Options: Health at risk | Relationships at risk | Values compromised | Time/energy depleted | Other

#### Closing

Summarize findings and identify 1-3 specific adjustments for the coming month.

---

## Check-in Template

### Month: _____________ | Date: _____________

---

### Question 1: Alignment Check

**What really matters to me right now:**

________________________________
________________________________
________________________________

**Are my Big Goals still aligned?**

| Goal | Status |
|------|--------|
| Professional #1: _____________ | Aligned / Needs adjustment |
| Professional #2: _____________ | Aligned / Needs adjustment |
| Professional #3: _____________ | Aligned / Needs adjustment |
| Personal #1: _____________ | Aligned / Needs adjustment |
| Personal #2: _____________ | Aligned / Needs adjustment |
| Personal #3: _____________ | Aligned / Needs adjustment |

**Adjustments needed:**

________________________________
________________________________

---

### Question 2: Systems Assessment

**Daily Systems consistency this month:**

| System | Days Completed | Level Distribution |
|--------|----------------|-------------------|
| _____________ | ___/30 | A: ___ B: ___ C: ___ |
| _____________ | ___/30 | A: ___ B: ___ C: ___ |
| _____________ | ___/30 | A: ___ B: ___ C: ___ |
| _____________ | ___/30 | A: ___ B: ___ C: ___ |

**What worked well:**

________________________________

**What got in the way:**

________________________________

**Systems changes for next month:**

________________________________

---

### Question 3: Anti-Goal Status

**Anti-Goal review:**

| Anti-Goal | Status |
|-----------|--------|
| ________________________________ | Green / Yellow / Red |
| ________________________________ | Green / Yellow / Red |
| ________________________________ | Green / Yellow / Red |
| ________________________________ | Green / Yellow / Red |

**Warning signs observed:**

________________________________

**Course corrections needed:**

________________________________

---

### Monthly Summary

**Biggest win this month:**

________________________________

**Biggest challenge this month:**

________________________________

**Focus for next month:**

1. ________________________________
2. ________________________________
3. ________________________________

**Checkpoint Goal progress:**

| Checkpoint | Target Date | Status |
|------------|-------------|--------|
| ________________________________ | _________ | On track / Behind / Ahead |
| ________________________________ | _________ | On track / Behind / Ahead |

---

### Quick Wins for Next Month

Three specific actions to take in the next 30 days:

1. [ ] ________________________________
2. [ ] ________________________________
3. [ ] ________________________________

---

## Monthly Check-in Schedule

- [ ] January (Last Friday: Jan 31)
- [ ] February (Last Friday: Feb 28)
- [ ] March (Last Friday: Mar 28)
- [ ] April (Last Friday: Apr 25)
- [ ] May (Last Friday: May 30)
- [ ] June (Last Friday: Jun 27)
- [ ] July (Last Friday: Jul 25)
- [ ] August (Last Friday: Aug 29)
- [ ] September (Last Friday: Sep 26)
- [ ] October (Last Friday: Oct 31)
- [ ] November (Last Friday: Nov 28)
- [ ] December (Last Friday: Dec 26)

---

## Reminders

**If Big Goals need major changes:**
- This is normal—life evolves
- Better to adjust than pursue outdated goals
- Use the Goal Decomposition template for new goals

**If Daily Systems aren't working:**
- Check if ABC levels are realistic
- Consider 30-for-30 approach to rebuild momentum
- Simplify—fewer systems done consistently beat more systems done sporadically

**If Anti-Goals are at risk:**
- This is the most important warning sign
- Course correct immediately
- Achieving goals at the cost of Anti-Goals is losing

**Remember:** The goal of monthly check-ins is small course corrections, not major overhauls. Catch the 1-degree errors before they become 60-mile misses.
